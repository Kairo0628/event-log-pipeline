...
- Adminer(`localhost:8080`)에 접속하면 웹을 통해 DB를 조작할 수 있습니다.
    - 데이터베이스 형식: `postgres`
    - 서버: `db`
    - 사용자 이름: `admin`
    - 비밀번호: `admin`
...
이벤트는 현실 사용자 패턴을 반영하여 출퇴근 및 저녁 시간에 가장 많은 이벤트를 생성하도록 하였습니다.
...
## 로그 저장
로그는 **postgreSQL**에 저장하였습니다. 해당 DB를 선택한 이유는 다음 네 가지가 있습니다.
